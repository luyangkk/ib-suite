...
follow the standalone guide at `$SKILL_ROOT/references/ib-trade-history-flex-query-setup.md`. The field
...
Set `data.base_currency` in `$WORKSPACE_ROOT/.ib-suite/config.yaml`. Credentials come only from
...
$WORKSPACE_ROOT/.ib-suite/venv/bin/python $SKILL_ROOT/ib-trade-history/scripts/configure_flex.py \
  --config $WORKSPACE_ROOT/.ib-suite/config.yaml --token-stdin \
...
```bash
$WORKSPACE_ROOT/.ib-suite/venv/bin/python $SKILL_ROOT/ib-trade-history/scripts/trade_history.py \
  --config $WORKSPACE_ROOT/.ib-suite/config.yaml \
...
```bash
$WORKSPACE_ROOT/.ib-suite/venv/bin/python $SKILL_ROOT/ib-trade-history/scripts/trade_history.py \
  --config $WORKSPACE_ROOT/.ib-suite/config.yaml --period mtd
```
